...
/**
 * DecryptedText — ported from React Bits (reactbits.dev, MIT License).
 * Source: github.com/DavidHDev/react-bits — TextAnimations/DecryptedText.
 *
 * Adaptations for this project:
 * - `motion/react` imports map to the installed `framer-motion` (same API).
 * - `useReducedMotion` renders plain text with no scrambling.
 * - Default scramble glyphs are hex-only (0-9, A-F), echoing packet hex dumps
 *   instead of the generic alphanumeric soup.
 * - Exposes `as`/`className` on the outer element so it can serve as a heading.
 */
...
export function DecryptedText({
  ...
  if (reduced) {
    return (
      <span className={cn(parentClassName, className)} {...(props as object)}>
        {text}
      </span>
    );
  }
      ...
      <span aria-hidden="true">
        {displayText.split("").map((char, index) => {
          ...
          return (
            <span
              key={index}
              className={isRevealedOrDone ? className : cn(className, encryptedClassName)}
            >
              {char}
            </span>
          );
